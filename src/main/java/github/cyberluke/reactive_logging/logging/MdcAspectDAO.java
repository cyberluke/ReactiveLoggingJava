...
public class MdcAspectDAO {
        ...
        return Mono.deferContextual(contextView -> {
            ...
            try (MDC.MDCCloseable mdcCloseable = MDC.putCloseable(MDC_KEY, contextKey)) {
                ...
                return ((Mono<?>) joinPoint.proceed())
                    .doOnSubscribe(subscription -> logger.info("Loading database for {}", camelCaseToSpaced(methodName)))
                    .doOnSuccess(result -> {
                        safelyCastToList(result).ifPresentOrElse(
                            list -> logger.info("{} successfully loaded from database. Number of items loaded: {}", camelCaseToSpaced(methodName), list.size()),
                            () -> logger.info("{} successfully loaded from database.", camelCaseToSpaced(methodName))
                        );
                    })
    ...
    public static <T> Optional<List<T>> safelyCastToList(Object obj) {
        if (obj instanceof List) {
            return Optional.of((List<T>) obj);
        }
